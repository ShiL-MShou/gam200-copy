...
\brief
This file contains the declaration of a class named ObjectAllocator which will
...
static const int DEFAULT_MAX_PAGES = 100;
...
class ObjectAllocator
{
  ...
  private:
    ...
    ObjectAllocator &operator=(const ObjectAllocator &oa) = default;
    ...
    // The list of pages
    GenericObject* _PageList;
    
    // The list of empty blocks
    GenericObject* _FreeList;
    
    // Creates a new page
    void AddNewPage(GenericObject* _nextpage = nullptr);  
    
    // Checks if the page is empty
    bool IsEmpty(GenericObject* _page);
    
    // Checks if the block is in the free list
    bool InFreeList(unsigned char* _block);
    
    // Removes all the blocks in the page from the freelist
    void RemovePage(GenericObject* _page);
};
